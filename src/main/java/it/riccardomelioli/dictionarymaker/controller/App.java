...
public class App implements Runnable {
    ...
    private int workStatus; // percentuale dove si � arrivati nel cicclo
    ...
    protected void generate(int keyLength, Vector<Character> symbolVector, PrintWriter txtOut, Boolean chckbxKeyLowerThanLength) {
        ...
        while (j <= keyLength) {
            ...
            Vector<Character> tmpKey = new Vector<>();    // si considera j perche la lunghezza della chiave � variabile, quando si considera le chiavi di lungh minore a keyLength
            while (tmpKey.size() < j) {
            ...
            while (tmpKeyTerminated) {    //  0037 - 9999 -> end 9999		// NON SI FERMA QUA
                ...
                workStatus = (int) (((float) cicleCounter) / totalKeyNumber * 100);    // percentuale dove si � arrivati
                ...
                for (int t = 0; t < tmpKey.size(); ++t) {
                    if (tmpKey.elementAt(t) != finalKey.elementAt(t))    // se vi � solo un elemento diverso dalla finale ritorna nel ciclo
    ...
    public void directTXT(final int keyLength,
                          final boolean chckbx_09,
                          final boolean chckbx_az,
                          final boolean chckbx_AZ,
                          final boolean chckbx_SpecialCharacter,
                          final boolean chckbxKeyLowerThanLength
    ) {

        // La generazione delle chiavi � distribuita su un altro thread
        final Thread threadGenerate = new Thread(new Runnable() {        // threadGenerate lavora al massimo
            @Override
            public void run() {
        ...
        updateViewTimer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                if (threadGenerate.isAlive()) {    // se il thread � attivo
